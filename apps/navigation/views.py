...
def beacon_sync(request: HttpRequest) -> HttpResponse:
    ...
    if result["status"] == "ok":
        ...
        messages.warning(request, result["message"] + " Grant it on the ESI Scopes page.")
    else:
...
def beacon_add(request: HttpRequest) -> HttpResponse:
    ...
    if not frm or not to or frm.system_id == to.system_id:
        messages.error(request, "Pick two different systems for the bridge.")
        return redirect("navigation:beacons")
    ...
    if not created:
        ...
        obj.save()
    messages.success(request, f"Jump bridge {a[1]} ⇄ {b[1]} saved.")
    return redirect("navigation:beacons")
...
def beacon_remove(request: HttpRequest, pk: int) -> HttpResponse:
    ...
    messages.success(request, "Jump bridge removed.")
    return redirect("navigation:beacons")
...
def jump_route_save(request: HttpRequest) -> HttpResponse:
    ...
    if not JumpPlannerConfig.active().allow_saved_routes:
        messages.error(request, "Saved routes are turned off.")
    ...
    if not origin or not dest:
        messages.error(request, "Plan a route first, then save it.")
        return redirect("navigation:jump_planner")
    ...
    messages.success(request, f"Saved route “{name}”.")
...
def jump_route_open(request: HttpRequest, pk: int) -> HttpResponse:
    ...
    if route.owner_id != request.user.id and not (
        ...
        messages.error(request, "You can't open that route.")
        return redirect("navigation:jump_routes")
    params = {
...
def jump_route_delete(request: HttpRequest, pk: int) -> HttpResponse:
    ...
    if route.owner_id == request.user.id or (
        ...
        route.delete()
        messages.success(request, "Route removed.")
    else:
        messages.error(request, "You can't remove that route.")
    return redirect("navigation:jump_routes")
...
def jump_route_watch(request: HttpRequest, pk: int) -> HttpResponse:
    ...
    if not route.watch_enabled:  # about to arm — cap the per-user watch fan-out
        ...
        if watched >= _MAX_WATCHED_ROUTES:
            messages.error(request, f"You can watch at most {_MAX_WATCHED_ROUTES} routes at once — "
                                    "turn one off first.")
            return redirect("navigation:jump_routes")
    ...
    messages.success(
        request,
        f"Camp/incursion watch {'on' if route.watch_enabled else 'off'} for “{route.name}”."
        + (" You'll be DMed when a threat appears on it." if route.watch_enabled else ""),
    )
    return redirect("navigation:jump_routes")
