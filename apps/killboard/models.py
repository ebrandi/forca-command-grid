...
class KillboardHistoryImport(TimeStampedModel):
    """KB-38: one tracked run of the one-click bulk history import (self-host adoption).

    A thin, cancellable wrapper over the existing ``import_everef_killmails`` /
    ``import_zkill_history`` management commands so a new instance can be populated from the
    setup wizard instead of the shell. Only ONE row may be *active* (pending or running) at a
    time — the launcher refuses a second concurrent import, so two backfills never race over
    the same killmails or double the network load. Cancel is best-effort: a director sets
    ``cancel_requested`` and the running command honours it between batches. Because the
    underlying commands are idempotent and resumable, a cancelled run simply leaves a
    consistent partial history that a later run continues from.
    """

    class Source(models.TextChoices):
        EVEREF = "everef", _("EVE Ref archives")
        ZKILL = "zkill", _("zKillboard history")
    ...
    # EVE Ref needs a date range; zKillboard walks the full history (from/to left null).
    from_date = models.DateField(null=True, blank=True)
    to_date = models.DateField(null=True, blank=True)
    state = models.CharField(
        max_length=16, choices=State.choices, default=State.PENDING, db_index=True
    )
    # Best-effort cancel flag: the running command re-reads it between batches and stops.
    cancel_requested = models.BooleanField(default=False)
    # Progress counters, refreshed as the run proceeds.
    matched = models.PositiveIntegerField(default=0)
    ingested = models.PositiveIntegerField(default=0)
    errors = models.PositiveIntegerField(default=0)
    killmail_count_before = models.PositiveIntegerField(default=0)
    killmail_count_after = models.PositiveIntegerField(default=0)
    # A bounded tail of the command's stdout/stderr for the progress panel (never the whole log).
    log_tail = models.TextField(blank=True)
    started_at = models.DateTimeField(null=True, blank=True)
    finished_at = models.DateTimeField(null=True, blank=True)
    ...
    class Meta:
        ordering = ["-created_at"]
        indexes = [
            models.Index(fields=["state", "-created_at"], name="kbimport_state_created_idx"),
        ]

    def __str__(self) -> str:
        return f"{self.get_source_display()} import ({self.state})"
    ...
    @classmethod
    def active(cls) -> "KillboardHistoryImport | None":
        """The single in-flight import (pending or running), or ``None``."""
        return cls.objects.filter(state__in=cls.ACTIVE_STATES).order_by("-created_at").first()
